    ...
    <div className="flex flex-col gap-6">
      ...
      <section className="rounded-lg border border-zinc-800 bg-zinc-900/60">
        <header className="border-b border-zinc-800 px-5 py-3">
          <h2 className="text-sm font-bold text-zinc-100">나의 포트폴리오</h2>
        ...
        {!session?.user ? (
          ...
          <>
            <div className="grid grid-cols-1 gap-3 border-b border-zinc-800 px-5 py-4 sm:grid-cols-3">
              <div>
                <p className="text-xs text-zinc-500">나의 총 자산 (현재 평가)</p>
                <p className="mt-0.5 text-2xl font-bold text-amber-400">
                  {formatKrw(totalNow)}원
                </p>
              </div>
              <div>
                <p className="text-xs text-zinc-500">총 매수금액</p>
                <p className="mt-0.5 text-2xl font-bold text-zinc-200">{formatKrw(totalBuy)}원</p>
              </div>
              <div>
                <p className="text-xs text-zinc-500">평가손익</p>
                <p className={`mt-0.5 text-2xl font-bold ${pnlColor(totalPnl)}`}>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-zinc-800 text-xs text-zinc-500">
                    <th className="px-5 py-2 text-left font-medium">코인</th>
                    <th className="px-2 py-2 text-right font-medium">수량</th>
                    <th className="px-2 py-2 text-right font-medium">매수단가</th>
                ...
                <tbody>
                  {rows.map(({ item, current, buyTotal, curTotal, pnl }) => (
                    <tr key={item.id} className="border-b border-zinc-800/60 last:border-0">
                      <td className="px-5 py-2 font-semibold text-zinc-200">{item.symbol}</td>
                      ...
                      <td className="px-2 py-2 text-right text-zinc-300">{formatKrw(buyTotal)}</td>
                      <td className="px-2 py-2 text-right text-zinc-300">{formatKrw(current)}</td>
