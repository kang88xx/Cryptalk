...
export default async function AttendancePage() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <h1 className="mb-1 text-lg font-bold text-zinc-100">출석체크</h1>
      <p className="mb-4 text-xs text-zinc-500">
        매일 출석하면 10포인트가 적립됩니다. (KST 기준 {today})
      </p>

      <section className="rounded-lg border border-zinc-800 bg-zinc-900/60 px-5 py-8 text-center">
        {!session?.user ? (
          ...
          <div>
            <p className="text-lg font-bold text-emerald-400">오늘 출석 완료 ✓</p>
            <p className="mt-1 text-xs text-zinc-500">이번 달 {myMonthCount}회 출석했습니다.</p>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-2">
            <AttendanceButton />
            <p className="text-xs text-zinc-500">이번 달 {myMonthCount}회 출석했습니다.</p>
          </div>
        )}
      </section>
      ...
      <section className="mt-4 rounded-lg border border-zinc-800 bg-zinc-900/60">
        <header className="border-b border-zinc-800 px-4 py-2.5">
          <h2 className="text-sm font-bold text-zinc-100">오늘의 출석부 ({todayLogs.length}명)</h2>
        </header>
        {todayLogs.length === 0 ? (
