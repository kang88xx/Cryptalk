// @NewListingsFeed 텔레그램 공개 채널 → 금일 신규 상장만 추출
...
function isPrivateIPv6(ip: string): boolean {
  ...
  if (/^fe[89ab]/.test(s)) return true; // 링크로컬 fe80::/10 (fe80~febf)
  if (/^f[cd]/.test(s)) return true; // 유니크 로컬 fc00::/7
  const dotted = s.match(/::ffff:(\d{1,3}(?:\.\d{1,3}){3})$/); // IPv4-mapped (점표기)
  if (dotted) return isPrivateIPv4(dotted[1]);
  const hex = s.match(/::ffff:([0-9a-f]{1,4}):([0-9a-f]{1,4})$/); // IPv4-mapped (16진)
  if (hex) {
...
// 공개 http(s) + 사설/루프백/메타데이터로 해석되지 않는 호스트만 허용 (best-effort)
async function isSafePublicUrl(raw: string): Promise<boolean> {
  ...
  // 원시 IPv6 리터럴은 전면 차단 (정상 거래소/뉴스 URL은 호스트명을 사용)
...
export type Listing = {
  ...
  detail: string; // 예: "listed on Bybit futures"
  text: string; // 디코딩된 원문 한 줄
  url: string | null; // 메시지 링크
...
function parseListings(html: string): Listing[] {
  const out: Listing[] = [];
  // 각 메시지 텍스트 블록을 순회하며, 바로 뒤에 오는 datetime / 메시지 링크를 매칭
...
async function extractScheduledTime(url: string | null): Promise<string | null> {
  ...
  try {
    ...
    const t = (await res.text())
      ...
      .replace(/\s+/g, " ");
    // 두 가지 어순 모두 대응
    const a = t.match(/([A-Z][a-z]{2,8})\.?\s+(\d{1,2}),?\s*(\d{4}),?\s+(\d{1,2}):(\d{2})\s*\(?UTC/); // Mon DD, YYYY HH:MM UTC
    const b = t.match(/(\d{1,2}):(\d{2})\s*\(?UTC\)?\s+on\s+([A-Z][a-z]{2,8})\.?\s+(\d{1,2}),?\s*(\d{4})/); // HH:MM UTC on Mon DD, YYYY
